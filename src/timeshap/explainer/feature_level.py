...
def local_feat(f: Callable[[np.ndarray], np.ndarray],
    ...
    if feature_dict.get("path") is None or not os.path.exists(feature_dict.get("path")):
        ...
        # Prefer filtering by the standard 'Entity' column if present
        if 'Entity' in feat_data.columns:
            feat_data = feat_data[feat_data['Entity'] == entity_uuid]
        # Backwards-compatibility: support files that used a custom entity_col name
        elif entity_col is not None and entity_col in feat_data.columns:
            feat_data = feat_data[feat_data[entity_col] == entity_uuid]
        elif len(feat_data.columns) == 4:
            # older per-instance export without entity — accept as-is
            pass
        else:
            # unexpected format
            raise ValueError
    ...
    if make_predictions:
        ...
        if file_path is not None:
            if os.path.exists(file_path):
                assert append_to_files, "The defined path for event explanations already exists and the append option is turned off. If you wish to append the explanations please use the flag `append_to_files`"
            else:
                if '/' in file_path:
        ...
        for rs in random_seeds:
            for ns in nsamples:
                for seq_idx, sequence in enumerate(data):
                    # Bind entity for each sequence: real entity if entity_col provided, else deterministic synthetic id
                    ...
                    for tol in tolerances_to_calc:
                        ...
                        if prev_pruning_idx == pruning_idx:
                            ...
                            # Ensure a consistent 'Entity' column name is present in the output
                            feat_data['Entity'] = entity
                            feat_data['Tolerance'] = tol
