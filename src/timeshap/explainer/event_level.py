...
def local_event(f: Callable[[np.ndarray], np.ndarray],
    ...
    if event_dict.get("path") is None or not os.path.exists(event_dict.get("path")):
        ...
        # If CSV contains an Entity column, always use it to filter by UUID.
        if 'Entity' in event_data.columns:
            event_data = event_data[event_data['Entity'] == entity_uuid]
        # Older format with entity_col name (if user provided a custom column name)
        elif entity_col is not None and entity_col in event_data.columns:
            event_data = event_data[event_data[entity_col] == entity_uuid]
        # If CSV has 4 cols (no entity), assume it's per-instance export or global; accept it
        ...
        for rs in random_seeds:
            for ns in nsamples:
                for seq_idx, sequence in enumerate(data):
                    # Determine entity value for this sequence:
                    # - use the real entity value when entity_col was provided,
                    # - otherwise use synthetic deterministic id (sequence index).
                    ...
                    for tol in tolerances_to_calc:
                        ...
                        if prev_pruning_idx == pruning_idx:
                            ...
                            # Always write the "Entity" column (use consistent output name)
